...
def train(model, train_loader, val_loader, loss_fn, optimizer, scheduler,
    ...
    history : dict  {key: list of per-epoch averages}
        Keys: 'total', 'photometric', 'smoothness', 'supervised', 'continuity',
              'val_total', 'val_epe'
    """
...
def plot_loss_history(history, cfg):
    ...
    Row 1: total | photometric | smoothness | laplacian | continuity
...
if __name__ == '__main__':
    ...
    # plot history
    if args.plot_results:
        history_path = 'outputs-pwcnet/train_history.json'
        with open(history_path, 'r') as file:
            full_history = json.load(file)
        total = len(full_history['total'])
        plot_loss_history(full_history, stages, cfg)
